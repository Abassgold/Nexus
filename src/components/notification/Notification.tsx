    ...
    <div className="fixed top-20 right-4 z-60 flex flex-col gap-2 pointer-events-none">
      {notifications.map((notif) => (
        <div
          key={notif.id}
          className={`
            pointer-events-auto flex items-center gap-3 w-80 p-4 
        ...
        >
          <div className="shrink-0">
            {notif.type === 'success' && <CheckCircleIcon size={20} className="text-stock-green" />}
            {notif.type === 'error' && <AlertCircleIcon size={20} className="text-red-500" />}
            {notif.type === 'info' && <InfoIcon size={20} className="text-accent" />}
          </div>
