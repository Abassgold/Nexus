...
import Acc2PurchaseModal from './Acc2PurchaseModal';
...
export function Acc2ProductRow({ product, index }: ProductRowProps) {
...
`}>
                ...
                <div className="flex md:items-center gap-4">
                    ...
                    <div className="min-w-0" title={product.description}>
        ...
        ${product.amount > 0 ? 'bg-stock-green/10 text-stock-green border border-stock-green/20' : 'bg-stock-grey/10 text-stock-grey border border-stock-grey/20'}
      `}>
        ...
        ${product.amount > 0 ? 'bg-accent text-surface-primary hover:bg-accent-hover' : 'bg-surface-tertiary text-txt-muted cursor-not-allowed border border-border-subtle'}
      `}>
                        <ShoppingCartIcon size={14} />
                        {product.amount > 0 ? 'Buy Now' : 'Sold Out'}
                    </button>
                </div>
            </div>
        </>
